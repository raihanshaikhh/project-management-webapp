...
const TASKS = [
  ...
  { id: 2, title: "Review PR #42",          project: "Website Redesign", status: "To Do",       due: "Apr 7" },
  { id: 3, title: "Write API endpoint docs", project: "API Integration",  status: "To Do",       due: "Apr 8" },
  { id: 4, title: "Update color tokens",     project: "Design System",    status: "Done",        due: "Apr 4" },
  { id: 5, title: "Set up CI pipeline",      project: "Mobile App v2",    status: "Done",        due: "Apr 3" },
];

const PROJECTS = [
  { name: "Website Redesign", color: "#3b82f6", completed: 6, total: 10 }, // blue-500
  { name: "Mobile App v2",    color: "#22c55e", completed: 3, total: 8  }, // green-500 (cleaner than emerald)
  { name: "Q3 Marketing",     color: "#f59e0b", completed: 8, total: 12 }, // amber-500
  { name: "Design System",    color: "#8b5cf6", completed: 5, total: 11 }, // violet-500
];
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Stats title="Open Tasks"  value="120" icon={<FaTasks           className="text-blue-400"   />} />
        <Stats title="In Progress" value="8"   icon={<RiProgress5Line  className="text-amber-400" />} />
        <Stats title="Completed"   value="32"  icon={<AiOutlineFileDone className="text-emerald-400" />} />
        <Stats title="Projects"    value="8"   icon={<GrProjects        className="text-violet-400" />} />
      </div>
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-4 py-4 transition-opacity ${
                    i < TASKS.length - 1 ? "border-b border-zinc-800/50" : ""
                  } ${done ? "opacity-60" : "opacity-100"}`}
                >
                  {/* Checkbox */}
                  <div
                    className={`mt-1 w-5 h-5 min-w-[20px] rounded-md border-2 flex items-center justify-center transition-all ${
                      done
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold truncate ${done ? "line-through text-zinc-500" : "text-zinc-100"}`}>
                      {task.title}
                    </p>
                    <p className="text-xs text-zinc-500 mt-1 font-medium">
                      <span className="text-zinc-400">{task.project}</span> • Due {task.due}
                    </p>
                  </div>

                  {/* Badge */}
                  <span className={`text-[10px] uppercase tracking-wider font-bold px-3 py-1 rounded-lg whitespace-nowrap ${BADGE[task.status]}`}>
                    {task.status}
                  </span>
                </div>
              );
          ...
          <div className="flex flex-col gap-2">
            {PROJECTS.map((proj, i) => {
              ...
              return (
                <div
                  key={proj.name}
                  className={`py-4 group ${i < PROJECTS.length - 1 ? "border-b border-zinc-800/50" : ""}`}
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs font-mono font-bold text-zinc-400 group-hover:text-white transition-colors">{pct}%</span>
                  </div>
                  
                  {/* Progress Bar Track */}
                  <div className="w-full bg-zinc-800/80 rounded-full h-2 overflow-hidden shadow-inner">
                    <div
                      className="h-full rounded-full transition-all duration-700 ease-out shadow-[0_0_8px_rgba(0,0,0,0.5)]"
                      style={{ 
                        width: `${pct}%`, 
                        background: proj.color,
                        boxShadow: `0 0 12px ${proj.color}44` 
                      }}
                    />
                  </div>
                  ...
                  <div className="flex justify-between mt-3">
                    ...
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-tight">
                        {proj.completed} <span className="text-zinc-600">/</span> {proj.total} Tasks
                    </p>
                  </div>
                </div>
              );
