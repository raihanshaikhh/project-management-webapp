...
const Dashboard = () => {
  ...
  if (loading)
    return (
      <p className="text-zinc-500 p-6 animate-pulse font-medium">
        Loading workspace...
      </p>
    );

  if (!dashboard)
    return <p className="text-zinc-500 p-6">No dashboard data</p>;
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Stats
          title="In Progress"
          value={dashboard?.taskCounts?.in_progress ?? 0}
          icon={<RiProgress5Line className="text-amber-400" />}
        />
        <Stats
          title="Completed"
          value={dashboard?.taskCounts?.done ?? 0}
          icon={<AiOutlineFileDone className="text-emerald-400" />}
        />
        <Stats
          title="Projects"
          value={dashboard?.projectCount ?? 0}
